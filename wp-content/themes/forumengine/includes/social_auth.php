...
class ET_FaceAuth extends ET_SocialAuth{
	...
	public function auth_facebook(){
		try {
			...
			$fb_appID      = et_get_option('et_facebook_key');
			$fb_secret_key = et_get_option('et_facebook_secret');
			//check user id with a access token
			$check_resp   = wp_remote_get('https://graph.facebook.com/me?fields=id&access_token='.$_POST['fb_token']);
			$check_resp   = json_decode(wp_remote_retrieve_body($check_resp));
			$check_userid = $check_resp->id;
			// check user vefified app
			$fb_app_token = wp_remote_get('https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id='.$fb_appID.'&client_secret='.$fb_secret_key.'&fb_exchange_token='.$_POST['fb_token']);
			$appResp      = array();
			$fb_app_token = wp_parse_str( wp_remote_retrieve_body($fb_app_token), $appResp );

			$data = $_POST['content'];
			//if userid is different
			if( $check_userid != $data['id'] || !isset($appResp['access_token']) || $appResp['access_token'] == '' ){
				$resp = array(
					'success' 	=> false,
			...
			if ( isset($data['id']) && $user = $this->get_user($data['id']) ){
				...
				$userdata 	= FE_Member::convert( $user );
				$nonce 		= array(
...
class ET_GoogleAuth extends ET_SocialAuth{
	...
	public function add_scripts(){
		...
		wp_localize_script( 'google_auth', 'google_auth', array(
			'appID' 		=> et_get_option('gplus_client_id'),
			'auth_url' 		=> home_url( '?action=authentication')
		) );
	}
		...
		try {
			...
			if(isset($_POST['gplus_token']) && isset($_POST['gplus_code'])){
				...
				if($userinfor->id !== $data['id']){
					$resp = array(
						'success' => false,
						'msg'     => __('Please login by using your Google plus account again!', ET_DOMAIN)
					);
				}
			} else {
				$resp = array(
