...
function et_get_page_link($pages, $params = array() , $create = true) {
	...
	$et_page_link = true;
	if(class_exists('CE_Options')){
		$option = new CE_Options;
		$et_page_link = $option->get_option('et_get_page_link');
	}elseif(class_exists('ET_JobOptions')){
    ...
    // page not existed
    if ($et_page_link && empty($pages) || !is_array($pages)) {
...
/**
 * Generate bot-hidden phone number
 */
function et_bot_hidden($string){
    if(is_string($string))
    {
        $result = '';
        ...
        {
            $result .=$char;
            $hidden_count = rand(1,5);
            for($index = 0; $index < $hidden_count; $index ++){
                $result .= '<span style="display: none">'. rand(0,9) .'</span>';
            }
        }
        return $result;
    }
    return new WP_Error('et_bot_hidden', __("The agument must be String",ET_DOMAIN));
}
