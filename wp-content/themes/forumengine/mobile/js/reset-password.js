...
ForumEngine.Views.CreatePass = Backbone.View.extend({
	...
	initialize: function(){
			this.resetpass_validator	= $('form#reset_pass').validate({
				...
				messages: {
					new_pass: fe_front.form_login_error_msg,
					re_pass: {
						required	: fe_front.form_login_error_msg,
						equalTo		: fe_front.form_login_error_repass
					}
				},
		...
		$.ajax({
			...
			success: function(resp){
				if(resp.success){
					...
					setTimeout(function(){ window.location = fe_front.redirect_reset_pass; }, 3000);
				} else {
