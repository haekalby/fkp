...
class AAM_UserActivity {
    ...
    public function mainNotice() {
        ...
        if (is_object($screen) && ($screen->id == $this->screen) && !$this->installed()) {
            ...
            echo 'Install free <b>AAM User Activity Helper</b> plugin to track more activities. ';
            echo '<a href="https://github.com/vasyltech/aam-user-activity-helper" target="_blank">Download plugin.</a>';
            echo '</p></div>';
        }
    }
    ...
    public function utilityList($list) {
        $list['track-activity'] = array(
            'title' => __('Track User Activities', AAM_KEY),
            'descr' => __('Allow AAM to track user activities and stores then in the database.', AAM_KEY),
            'value' => AAM_Core_Config::get('track-activity', true)
        );
    ...
    public function getHooks() {
        if (is_null($this->hooks)) {
            ...
            $this->hooks = array_merge(
                    $default, $custom, is_array($config) ? $config : array()
            );
        }
